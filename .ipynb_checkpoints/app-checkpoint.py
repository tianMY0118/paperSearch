...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [paperSearch] %(levelname)s - %(message)s",
    handlers=[logging.StreamHandler(sys.stdout)]
)
logger = logging.getLogger("paperSearch")
...
def search_papers(query: str, max_results: int):
    ...
    papers = []
    output = f"📚 paperSearch 检索结果（关键词: {query}）\n\n"
...
def export_results(format: str):
    ...
    base_name = "paperSearch_export"
    ext_map = {"Text": "txt", "Word": "docx", "PDF": "pdf", "Excel": "xlsx"}
    filename = f"outputs/{base_name}.{ext_map.get(format, 'txt')}"

    watermark = "\n— 导出自 paperSearch 智能学术助手 (https://yourwebsite.com) —\n"

    try:
        if format == "Word":
            doc = Document()
            doc.add_heading("paperSearch 学术论文导出报告", 0)
            ...
            draw_line("paperSearch 学术论文导出报告", size=14, spacing=20)
            y -= 10
            for i, p in enumerate(papers):
        ...
        with open("paperSearch_export_log.json", "w", encoding="utf-8") as f:
...
# ===== Gradio 界面 =====
with gr.Blocks(title="paperSearch") as demo:
    gr.Markdown("## 🧠 paperSearch — 智能学术文献助手")
